...
| `data.js` | The prompt database: `const promptDB`, keyed `1..80`, each with tiers `a`/`b`/`c` (first/second/third visit). Also `const meaningTable` — the 100-word Meaning Oracle list (1-indexed by a d100 roll) — and `const setupSuggestions` (`names`/`skills`/`resources`/`characters`), the concrete example pools behind the setup wizard's 🎲 helpers (incl. `marks`) — and `const memoryTemplates` (`themes`/`life`/`combine`/`turning`), the sentence-starter templates behind 💡 Sentence starter and Surprise me. |
...
| `sw.js` | Service worker. `CACHE_NAME` = `vampire-chronicle-v20`. Precaches assets (incl. `assets/*.wav` and `assets/icon-*.png`), deletes old caches on activate, network-first for navigations + same-origin html/js/css/json (avoids version skew), stale-while-revalidate for other assets. **Does not `skipWaiting()` on install** — it waits so the page can offer "tap to update", and calls `skipWaiting()` only on a `SKIP_WAITING` message. |
...
- `memories` / `diary` (`{ id, theme, experiences[], memState, lost }` — `lost`
  ...
  **compact** array (≥1, up to 3 — or 5 when Vast); trailing empties are trimmed
  by `normMem`, and rows are added/removed via `addExperience`/`removeExperience`. Each Experience
  renders as an **auto-growing `<textarea>`** (`autoGrow`/`autoGrowAll`, re-fit on
  render and on tab-show) so a whole paragraph stays visible without scrolling.
...
1. **Setup wizard** (`#setupWizard`, 8 steps) rebuilds the rules-faithful vampire
   ...
   Meaning Oracle (`sparkInto`) **and** offer 💡 **Sentence starter**
   (`templateInto` → `memoryTemplates` + `TYOV.fillTemplate`, which names the
   player's own traits via `currentSetupTraits`). All render **tappable chips**
   (`chipHtml`/`chipRowHtml` → `applySuggestion`) that drop the text into the
   first empty field of that step (falling back to the last-focused field);
   textareas with existing text insert at the caret. Each chip row carries a ✕
   (`dismissChips`); tapping the 🎲/💡 button again re-rolls the row in place.
   **✨ Surprise me** (`surpriseMe`) fills every blank field across all 8 steps
   from the pools/templates and jumps to step 8 (`gotoStep(8, true)` — the
   `skipValidate` arg) for review before Begin. `gotoStep`/`validateStep` require every field before advancing;
...
4. **Guided prompt actions** (`promptCheckSkill`/`promptLoseResource`/
   `promptKillCharacter`): open a **hovering picker popover** (`showTraitPicker`→
   `traitPickerHTML`/`traitPickerRow`, anchored under the button by
   `positionTraitPicker`, centered on ≤520px screens) listing the relevant traits.
   Each row (`pickTrait`) toggles state live — check/un-check a Skill (via
   `setSkillChecked`), lose/restore a Resource, or kill/revive a Character
   (both via `toggleLoseEntity`, Diary-aware) — so you can also **un-select** a
   mistoggle. The **Kill-a-Character** picker (kind `characters`) lists **all**
   Characters (mortals first, each tagged Mortal/Immortal via `traitPickerRow`'s
   `tag`), since most kill prompts say "kill a Character" (Immortals included) and
   only a few specify a mortal; it also offers a `+ New mortal Character` row
   (`createMortalFromPicker`) for the rules' "create a mortal if none are
...
`CACHE_NAME` in `sw.js` (currently `-v20`). Bumping it is also what makes the
